...
{
	std::string result;
	static const char bet[] = { "ACBDEFGHIJKLMNOPQRSTUVWXYZabcdefgihjklnmopqrstuvwxyz" };
...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			HWND window = ::CreateWindow(windowClass.lpszClassName, RandomString().c_str(), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL );
